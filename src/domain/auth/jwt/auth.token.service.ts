...
export class AuthTokenService {
  ...
  async reissueAccessToken(tokenDto: AuthTokenDto) {
    if (!tokenDto.refreshToken)
      throw new UnauthorizedException('존재하지 않은 토큰');

    // 1. DB에 저장된 리프레시 토큰이 유효한지 검증
    const tokenEntity = await this.tokenRepo.findByToken(tokenDto.refreshToken);
    if (!tokenEntity) {
    ...
    const payload = { email: tokenEntity.user.email };
    ...
    return { accessToken, refreshToken: tokenDto.refreshToken };
  }
}
